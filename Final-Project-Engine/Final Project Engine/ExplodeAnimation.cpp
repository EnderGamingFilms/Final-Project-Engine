...
EnemyExplode::EnemyExplode(float xPos, float yPos, float len)
{
	...
	shad.Init("basic.shader", fourF, 0);
	shad.Create("u_Color");
	shad.setRgb(1.0, 1.0, 1.0);
	shad.Use();
	shad.UpdateUniform();
	shad.unBind();
...
void EnemyExplode::Init(float xPos, float yPos, float len)
{
	this->timer = ENEMY_EXPLODE_TIMER;
	this->midOffset = 0;
...
void EnemyExplode::Draw()
{
	...
	float localX[CURVE_POINT_COUNT] = { 0 }, localY[CURVE_POINT_COUNT] = { 0 }, localXflat[CURVE_POINT_COUNT] = { 0 }, localYflat[CURVE_POINT_COUNT] = { 0 };
	float tempX, tempY;
	for (int i = 0; i < CURVE_POINT_COUNT; i++) //Initialize for first quadrant (parallel to x - axis
	{
	...
	glBegin(GL_POLYGON); //Q1 - 0
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localXflat[i] + this->midpoint[0]) * SQ_ASPECT_RATIO, localYflat[i] + this->midpoint[1]);
	glEnd();
	glBegin(GL_POLYGON);
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localXflat[i] + this->midpoint[0]) * SQ_ASPECT_RATIO, localYflat[i] * -1 + this->midpoint[1]);
	glEnd();

	glBegin(GL_POLYGON); //Q1 - 45
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
	...
	glBegin(GL_POLYGON); //Q2 - 0
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localXflat[i] + this->midpoint[0]) * SQ_ASPECT_RATIO, localYflat[i] + this->midpoint[1]);
	glEnd();
	glBegin(GL_POLYGON);
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localXflat[i] * -1 + this->midpoint[0]) * SQ_ASPECT_RATIO, localYflat[i] + this->midpoint[1]);
	glEnd();

	glBegin(GL_POLYGON); //Q2 - 45
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localX[i] + this->midpoint[0]) * SQ_ASPECT_RATIO, localY[i] + this->midpoint[1]);
	glEnd();
	glBegin(GL_POLYGON);
	...
	for (int i = 0; i < CURVE_POINT_COUNT; i++) //Rotate 90 deg ccw to quadrant 3
	{
		...
		tempX = localXflat[i];
		tempY = localYflat[i];
		localXflat[i] = tempY * -1;
		localYflat[i] = tempX;
	}
	glBegin(GL_POLYGON); //Q3 - 0
	for (int i = 0; i < CURVE_POINT_COUNT && (i <= 1 || this->curve[i - 1] != 0); i++)
		glVertex2f((localXflat[i] + this->midpoint[0]) * SQ_ASPECT_RATIO, localYflat[i] + this->midpoint[1]);
	glEnd();
	glBegin(GL_POLYGON);
